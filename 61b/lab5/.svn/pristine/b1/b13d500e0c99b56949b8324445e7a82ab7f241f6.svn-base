...
public abstract class Creature extends Occupant {
    ...
    /** Converts a Map<Direction, String> N to a list of all neighbors of the
     * specified NAME.*/
    public List<Occupant> getNeighborsOfName(Map<Direction, Occupant> n,
                                                String name) {
        List<Occupant> L = new ArrayList<>();
        for(Occupant occupant : n.values()) {
            String occupantName = occupant.name();
