...
class Maker {
    ...
    /** Keeps track of only targets and not files. */
    private List<String> _realTargets = new ArrayList<>();
    ...
    class MakeTraversal extends DepthFirstTraversal {
        ...
        protected boolean shouldPostVisit(int v0) {
            if (_realTargets.contains(_depends.getLabel(v0).getTarget())
                    || _depends.outDegree(v0) > 0) {
                return true;
            }
            return false;
        }
