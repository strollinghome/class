...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new IllegalStateException("Already has dependencies");
        } else {
    ...
    private void checkFinishedDependencies() {
        Iteration<Integer> myDependencies = _depends.successors(_vertex);
        Rule dependency;
        for (Integer i : myDependencies) {
            dependency = _depends.getLabel(i);
            if (dependency.isUnfinished() && _depends.outDegree(i) > 0) {
                throw new IllegalStateException("Unfinished dependency");
            }
        }
    }
    ...
    private boolean outOfDate() {
        return _time == null || _maker.getCurrentTime() > _time;
    }
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            ...
            System.out.println(_commands.get(0));
        }
        _finished = true;
    }
