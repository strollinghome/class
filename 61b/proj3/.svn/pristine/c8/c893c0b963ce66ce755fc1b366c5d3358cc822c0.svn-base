...
class Trip {
    ...
    void makeTrip(String[] dests) {
        ...
        for (int i = 1; i < dests.length; i += 1) {
            Integer from = _sites.get(dests[i - 1]);
            Integer to = _sites.get(dests[i]);
            if (from == null) {
            ...
            if (d0.fullName().equals(d1.fullName())
                ...
                if (i == segment.size() - 3) {
                    System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                                    seq, next, d1.fullName(), miles,
                                    dest);
                    seq = seq + 1;
                }
            } else if (i == segment.size() - 2) {
                System.out.printf("%d. Take %s %s for %.1f miles.%n", seq,
                                    current, d0.fullName(), miles);
                seq = seq + 1;
                miles = next.length();
                System.out.printf("%d. Take %s %s for %.1f miles to %s.%n", seq,
                                    next, d1.fullName(), miles, dest);
            } else {
                System.out.printf("%d. Take %s %s for %.1f miles.%n", seq,
                                    current, d0.fullName(), miles);
                seq = seq + 1;
