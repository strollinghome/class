...
public abstract class ShortestPaths {
    ...
    public void setPaths() {
        ...
        double weight;
        int vertex;
        _traversed = new ArrayList<>();
        reset();
        setWeight(_source, 0);
        _fringe.add(_source);
        while (!_fringe.isEmpty()) {
            vertex = _fringe.remove();
            _traversed.add(vertex);
            if (vertex == _dest) {
                break;
            }
    ...
    private class VertexComparator<Integer> implements Comparator {
        ...
        public int compare(Object v, Object w) {
            int v0 = (int) v;
            int v1 = (int) w;
            if (getWeight(v0) + estimatedDistance(v0)
                    == getWeight(v1) + estimatedDistance(v1)) {
                return 0;
            } else if (getWeight(v0) + estimatedDistance(v0)
                    < getWeight(v1) + estimatedDistance(v1)) {
                return -1;
            } else {
                return 1;
    ...
    /** Resets the weight of all untraversed nodes to infinity. */
    private void reset() {
        Iteration<Integer> vertices = _G.vertices();
        for (Integer i : vertices) {
            if (!_traversed.contains(i)) {
    ...
    /** Keeps track of all traversed nodes. */
    private List<Integer> _traversed;
