 ...
 *
 *  @author Josh Hug and Carlos Flores
 */
...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int i = 1; i < m.length; i++) {
            for (int j = 0; j < m[0].length; j++) {
                if (j == 0) {
                    am[i][j] += Math.min(am[i - 1][j], am[i - 1][j + 1]);
                } else if (j == m[0].length - 1) {
                    am[i][j] += Math.min(am[i - 1][j - 1], am[i - 1][j]);
                } else {
                    am[i][j] += smallest(am[i - 1][j - 1], am[i - 1][j],
                            am[i - 1][j + 1]);
                }
            }
        }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) {
            ...
            return accumulateVertical(reverseMatrix(m));
        }
    }

    /** Reverses and returns a matrix M from Orientation.HORIZONTAL to
