...
class NuProcedure(LambdaProcedure):
    ...
    def evaluate_arguments(self, arguments_list, env):
        return arguments_list.map(lambda call: Thunk(nil, call, env))
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        bound_name = target.first
        if isinstance(bound_name, SchemeNumber):
            raise SchemeError 
        new_vals = Pair(target.second, vals.second)
        lambda_function = do_lambda_form(new_vals, env)[0]
        env.define(bound_name, lambda_function)
        return bound_name, None
    else:
...
def do_let_form(vals, env):
    ...
    for expression in bindings:
        names = Pair(expression.first, names)
        values = Pair(scheme_eval(expression.second.first, env), values)  
...
def do_and_form(vals, env):
    ...
    if len(vals) > 0:
        for index in range(len(vals) - 1):
            evaluated_expr = scheme_eval(vals[index], env)
...
def do_or_form(vals, env):
    ...
    for expression in vals:
        evaluated_expr = scheme_eval(expression, env)
        if evaluated_expr:
            return evaluated_expr, None
    return scheme_false, None
...
def check_formals(formals):
    ...
    for parameter in formals:
        if not scheme_symbolp(parameter) or parameter in parameter_list:
            raise SchemeError
