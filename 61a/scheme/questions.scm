...
(define (list-partitions total max-pieces max-value)
  (define (count-partition total max-pieces max-value partitions)
    (cond 
     ((= total 0) (list partitions))
     ((< total 0) nil)
     ((< max-pieces 1) nil)
     ((< max-value 1) nil)
     (else (append (count-partition total  max-pieces (- max-value 1) partitions) 
		   (count-partition (- total max-value) (- max-pieces 1) max-value (cons max-value partitions))))))
  (count-partition total max-pieces max-value '()))
...
(define (tree-sums tree)
 (cond
  ...
  (else (map (lambda (n) (+ n (entry tree))) (flatten (map tree-sums (children tree))))))) 
