...
class HarvesterAnt(Ant):
    ...
    def action(self, colony):
        ...
        colo ny -- The AntColony, used to access game state information.
        """ 
        colony.food += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        self.armor -= amount
        if self.armor <= 0:    
            bee_set = set(self.place.bees[::])
            while len(bee_set) != 0:
                current_bee = bee_set.pop()
                current_bee.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        self.queen_number = QueenAnt.number_queens
    ...
    def action(self, colony):
        ...
        if self.queen_number > 1:
            ...
            self.queen_ant_damage(True, colony)
            self.queen_ant_damage(False, colony)

    def queen_ant_damage(self, direction, colony):
        if direction:
            ant_location = self.place
        else:
            ant_location = self.place.entrance
        while ant_location.exit and ant_location.entrance: 
            if ant_location.ant not in QueenAnt.ants_doubled:
                if type(ant_location.ant) == BodyguardAnt:
                    if ant_location.ant.ant and ant_location.ant.ant != self:
                        ant_location.ant.ant.damage = (ant_location.ant.ant.damage * 2)
                        QueenAnt.ants_doubled.append(ant_location.ant.ant)
                if ant_location.ant and ant_location.ant != self:
                    ant_location.ant.damage = (ant_location.ant.damage * 2)
                    QueenAnt.ants_doubled.append(ant_location.ant)
            if direction:
                ant_location = ant_location.exit
            else:
