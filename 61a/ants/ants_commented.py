...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        ###Next Place instance sets entrance to previous Place instance.
        if self.exit:
            exit.entrance = self  ###Exits (left Place instance) entrance is set to yourself (self).
...
class Insect:
    ...
    watersafe = False ###Class attribute default setting False.
...
class Bee(Insect):
    ...
    name = 'Bee'
    watersafe = True ###Bees can fly, therefore WATERSAFE default value set to True.
...
class FireAnt(Ant):
    ...
    name = 'Fire'
    damage = 3
    food_cost = 4 ###Default cost
    implemented = False
    ###Place is a list holding different instance (bees, ants etc).
    def reduce_armor(self, amount):
        if self.armor <= 0:
            place.bees.armor -= damage ###Reduce armor of the bees in place by damage amount.
