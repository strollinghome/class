...
def shuffle(cards):
    ...
    shuffled_deck = []
    lower_half = cards[int(len(cards) / 2):]
    for i in range(int(len(cards) / 2)):
        shuffled_deck.append(cards[i])
        shuffled_deck.append(lower_half[i])
...
def make_withdraw(balance, password):
    ...
    """ 
    incorrect_passwords = []
    transaction = {'balance_amount': balance, 'code': password}
    def withdraw_password(amount, password):
        nonlocal transaction
        nonlocal incorrect_passwords
        if transaction['code'] == str(password) and len(incorrect_passwords) < 3:
            if transaction['balance_amount'] < amount:
                return 'Insufficient funds' 
            transaction['balance_amount'] = transaction['balance_amount'] - amount
            return transaction['balance_amount']
        else:
            incorrect_passwords.append(str(password))
            if len(incorrect_passwords) >= 4:
                return 'Your account is locked. Attempts: ' + str(incorrect_passwords[:3])
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    def joint_account(amount, guess_password):
        nonlocal withdraw, old_password, new_password
        if guess_password in [old_password, new_password]:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, guess_password)

    trial = withdraw(0, old_password)
    if type(trial) == str:
        return trial
    else:
