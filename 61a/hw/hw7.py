...
def apply(operator_name, shape):
    ...
    12
    """
    type_object = {('perimeter', Square): lambda shape: shape.side*4, 
                    ('area', Square): lambda shape: shape.side**2, 
                    ('perimeter', Rect): lambda shape: 2*shape.width + 2*shape.height, 
                    ('area', Rect): lambda shape: shape.width*shape.height}
    key = (operator_name, type(shape))
    return type_object[key](shape)
...
def drlist_adjoin(s, v):
    ...
    if s is Rlist.empty:
        ...
        s.first, s.rest, = v, Rlist(s.first, s.rest)
        return s
    ...
    def __next__(self):
        while len(self._work_queue) > 0:
            ...
            if isinstance(subtree_or_label, EmptyBinTree):
                pass
            elif isinstance(subtree_or_label, BinTree):
...
def partial_sequence_to_tree(s, n):
    ...
    (tree_left, end) = partial_sequence_to_tree(s, left_size)
    (label, end) = end.first, end.rest
    (tree_right, end) = partial_sequence_to_tree(end, right_size)
    return (BinTree(label, tree_left, tree_right), end)
...
def mario_number(level):
    ...
    if level == level[-1]:
        return 1    
    current, next = level[0], level[1]
    if current == 'P':
        return 0
    elif next == 'P':
        return mario_number(level[2:])
    return mario_number(level[1:]) + mario_number(level[2:]) if len(level) > 2 else 1
# Q7.
