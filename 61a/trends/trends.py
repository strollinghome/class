...
def make_tweet_fn(text, time, lat, lon):
    ...
    38
    """
    content = {'text': text, 'time': time, 'lat':lat, 'lon': lon}
    def tweet(key):
        return content[key]
    return tweet
...
def analyze_tweet_sentiment(tweet):
    ...
    if sentiment_list == []:
        return average 
    total, length, = sum(sentiment_list), len(sentiment_list)
...
def find_centroid(polygon):
    ...
    area, centroid_x, centroid_y = 0, 0, 0
    for i in range(len(polygon) - 1):
        area += ((latitude(polygon[i]) * longitude(polygon[i + 1])) - (latitude(polygon[i +1]) * longitude(polygon[i])))
        centroid_x += (latitude(polygon[i]) + latitude(polygon[i + 1])) * (latitude(polygon[i]) * longitude(polygon[i + 1]) - latitude(polygon[i + 1]) * longitude(polygon[i]))
        centroid_y += (longitude(polygon[i]) + longitude(polygon[i + 1])) * (latitude(polygon[i]) * longitude(polygon[i + 1]) - latitude(polygon[i +1]) * longitude(polygon[i]))
    area = (area * .5)
    if area == 0:
        return (latitude(polygon[0]), longitude(polygon[0]), 0)   
    centroid_x = centroid_x / (area * 6)
    centroid_y = centroid_y / (area * 6)
...
def find_state_center(polygons):
    ...
    -156.21763
    """
    total_area, x_total, y_total = 0, 0, 0
    for i in range(len(polygons)):
        total_area += find_centroid(polygons[i])[2]
        x_total += find_centroid(polygons[i])[0] * find_centroid(polygons[i])[2]
        y_total += find_centroid(polygons[i])[1] * find_centroid(polygons[i])[2]
    x_total, y_total = x_total / total_area, y_total / total_area
    return make_position(x_total, y_total)
...
def group_tweets_by_state(tweets):
    ...
    """
    #return a dictionary with state names as keys and a list of tweets

    tweets_by_state = {}
    state_centers = {state: find_state_center(us_states[state]) for state in us_states.keys()}
    for tweet in tweets:
        closest = 10 ** 100
        name = ' '
    ...
    """"for i in us_states:
        state_centers[i] = find_state_center(us_states[i])
    for tweet in tweets:
        tweet_distances = {}
        for state in us_states:
            tweet_distances = geo_distance(tweet_location(tweet), state_centers[state])
        closest_state = min(tweet_distances)
...
def average_sentiments(tweets_by_state):
    ...
    for key in tweets_by_state.keys():
        tweets_state = tweets_by_state[key]
        total_tweets = 0
        for i in range(len(tweets_state)):
            if has_sentiment(analyze_tweet_sentiment(tweets_state[i])) == False:
                tweets_state[i] = 0
            else:
                total_tweets += 1
                tweets_state[i] = sentiment_value(analyze_tweet_sentiment(tweets_state[i]))
        if total_tweets != 0:
            averaged_state_sentiments[key] = sum(tweets_state) / total_tweets
