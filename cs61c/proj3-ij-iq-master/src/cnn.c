...
uint64_t conv0_time = 0;
uint64_t conv3_time = 0;
uint64_t conv6_time = 0;
uint64_t relu1_time = 0;
uint64_t relu4_time = 0;
uint64_t relu7_time = 0;
uint64_t pool2_time = 0;
uint64_t pool5_time = 0;
uint64_t pool8_time = 0;
uint64_t fc_time = 0;
          ...
          for(int ax=0; ax < l_out_sx; x += xy_stride, ax++) {
            ...
            for(int fy = 0; fy < f_sy; fy++) {
              ...
              for(int fx = 0; fx < f_sx; fx++) {
                int ox = x + fx;
                int f_sx_fy_fx = ((f_sx_fy)+fx) * f_depth;
                int vsx_oy_ox = (vsx_oy + ox) * v_depth;
                if(oy >= 0 && oy < V_sy && ox >=0 && ox < V_sx) {
                  __m256d vec_f0 = _mm256_loadu_pd(&cacher_f[f_sx_fy_fx]);
                  __m256d vec_f1 = _mm256_loadu_pd(&cacher_f[f_sx_fy_fx+4]);
                  __m256d vec_f2 = _mm256_loadu_pd(&cacher_f[f_sx_fy_fx+8]);
                  __m256d vec_f3 = _mm256_loadu_pd(&cacher_f[f_sx_fy_fx+12]);
                  __m256d vec_f4 = _mm256_loadu_pd(&cacher_f[f_sx_fy_fx+16]);
                  __m256d vec_v0 = _mm256_loadu_pd(&cacher_V[vsx_oy_ox]);
                  __m256d vec_v1 = _mm256_loadu_pd(&cacher_V[vsx_oy_ox+4]);
                  __m256d vec_v2 = _mm256_loadu_pd(&cacher_V[vsx_oy_ox+8]);
                  __m256d vec_v3 = _mm256_loadu_pd(&cacher_V[vsx_oy_ox+12]);
                  __m256d vec_v4 = _mm256_loadu_pd(&cacher_V[vsx_oy_ox+16]);
                  vec_f0 = _mm256_mul_pd(vec_f0, vec_v0);
                  vec_f1 = _mm256_mul_pd(vec_f1, vec_v1);
                  vec_f2 = _mm256_mul_pd(vec_f2, vec_v2);
                  vec_f3 = _mm256_mul_pd(vec_f3, vec_v3);
                  vec_f4 = _mm256_mul_pd(vec_f4, vec_v4);
                  vec_f0 = _mm256_add_pd(vec_f0, vec_f1);
                  vec_f1 = _mm256_add_pd(vec_f2, vec_f3);
                  vec_f0 = _mm256_add_pd(vec_f0, vec_f1);
                  vec_f0 = _mm256_add_pd(vec_f0, vec_f4);
                  // a += vec_f0[0] + vec_f0[1] + vec_f0[2] + vec_f0[3]; THIS WORKS
                  double arr_ALL[4];
                  _mm256_storeu_pd(arr_ALL, vec_f0);
                  a += arr_ALL[0] + arr_ALL[1] + arr_ALL[2] + arr_ALL[3];
                }
              }
            }
            a += l->biases->w[d];
          ...
          for(int ax=0; ax < l_out_sx; x += xy_stride, ax++) {
            ...
            for(int fy = 0; fy < f_sy; fy++) {
              ...
              for(int fx = 0; fx < f_sx; fx++) {
                ...
                if(oy >= 0 && oy < V_sy && ox >=0 && ox < V_sx) {
                  double local_sum = 0;
                  local_sum += cacher_f[f_sx_fy_fx] * cacher_V[vsx_oy_ox];
                  local_sum += cacher_f[f_sx_fy_fx+1] * cacher_V[vsx_oy_ox+1];
                  local_sum += cacher_f[f_sx_fy_fx+2] * cacher_V[vsx_oy_ox+2];
                  a += local_sum;
                }
              }
            }
              ...
              for(int fx = 0; fx < f_sx; fx++) {
                ...
                if(oy >= 0 && oy < V_sy && ox >=0 && ox < V_sx) {
                  ...
                  for(int fd=0; fd<f_depth; fd++) {
                   local_sum += cacher_f[f_sx_fy_fx+fd] * cacher_V[vsx_oy_ox+fd];
                  }
                  a += local_sum;
                }
              }
...
void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
  batch_t* batch = make_batch(net, 16);
  ...
  for (i; i<n/2; i+=8) { // divisor * increment = batch size
    copy_vol(batch[0][factor * 2], input[i * 2]); // original
    copy_vol(batch[0][factor * 2 + 1], input[i * 2 + 1]);
    net_forward(net, batch, factor * 2, factor * 2 + 1); // original
    output[i * 2] = batch[11][factor * 2]->w[CAT_LABEL]; // original
    output[i * 2 + 1] = batch[11][factor * 2 + 1]->w[CAT_LABEL];
  }
  }
  // for(int i=n/num_threads*num_threads; i < n; i++) {
  //   copy_vol(batch[0][0], input[i]); // original
  //   net_forward(net, batch, 0, 0); // original
  //   output[i] = batch[11][0]->w[CAT_LABEL]; // original
  // }

  // printf("conv0:    %lf ms\n", ((double)conv0_time) / 1000);
  // printf("relu1:    %lf ms\n", ((double)relu1_time) / 1000);
  // printf("pool2:    %lf ms\n", ((double)pool2_time) / 1000);
  // printf("conv3:    %lf ms\n", ((double)conv3_time) / 1000);
  // printf("relu4:    %lf ms\n", ((double)relu4_time) / 1000);
  // printf("pool5:    %lf ms\n", ((double)pool5_time) / 1000);
  // printf("conv6:    %lf ms\n", ((double)conv6_time) / 1000);
  // printf("relu7:    %lf ms\n", ((double)relu7_time) / 1000);
  // printf("pool8:    %lf ms\n", ((double)pool8_time) / 1000);
  // printf("fc:      %lf ms\n", ((double)fc_time) / 1000);
  // printf("softmax: %lf ms\n", ((double)softmax_time) / 1000);
